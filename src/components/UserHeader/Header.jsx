...
function Header() {
  ...
  const [role, setRole] = useState(
    getLocalStorage("user") ? getLocalStorage("user") : "United States"
  );
  ...
  useEffect(() => {
    const storedCountry =
      getLocalStorage("selectedCountry")?.Country || "United States";
  ...
  const shouldHideHeader = location.pathname.includes('/avatar/experience') && location.search.includes('tab=avathons');
    ...
    <>
      ...
      <header className="flex justify-between items-center p-3">
        ...
        <div className="cursor-pointer flex gap-4 items-center">
         ...
         {shouldHideHeader ?<button onClick={()=>navigate("/avatar/create-avathons")} className="bg-black text-[12px] text-white px-4 py-2 rounded hover:bg-gray-800">Add Other Avathons</button>
: <HeaderNavigation />}
        </div>
      </header>
    </>
