...
const Home = () => {
  ...
  const [country, setCountry] = useState(getLocalStorage("selectedCountry"));
    ...
    <>
      ...
      <div className="container">
        ...
        <div className="lg:overflow-x-auto lg:overflow-y-hidden border-b">
          <div className="flex border-b">
            {tabs.map(({ key }) => (
              <button
                key={key}
                className={`px-4 py-2 text-sm font-medium border-b-2 ${activeTab === key ? "border-primaryColor-900 text-primaryColor-900 font-bold" : "border-transparent text-gray-500 hover:text-gray-700"}`}
        ...
        <div className="-mx-4">
          ...
          >
            <div className="my-10 grid grid-cols-4 lg:grid-cols-2 sm:grid-cols-1 xl:grid-cols-2 gap-4">{localData?.length !== 0 ? localData.map((product) => <ExperienceList key={product._id} product={product} />) : <h1 className="font-medium text-sm">No Data Found</h1>}</div>
          </InfiniteScroll>
        </div>
      </div>
      ...
      <InstantLiveRequestModal instanreq={instant} role={"user"} instantLiveModalState={instantLiveModalState} setInstantLiveModalState={setInstantLiveModalState} />
      <MeetingNotification show={showNotification} onClose={handleCancel} onJoin={handleJoin} data={item} />
    </>
  );
};
