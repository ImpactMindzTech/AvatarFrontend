...
import { avtrateTourApi, getmeetdata, rateTourApi } from "@/utills/service/userSideService/userService/UserHomeService";
...
let meetId = localStorage.getItem('meet');
  ...
  const submitReview = async () => {
    ...
    try {
      ...
      const res = await avtrateTourApi(id, body);
    ...
    // }   
      ...
      if (res?.isSuccess) {
        ...
        if (tipMoney === 0) {
          ...
          navigate("/user/avttip", { state: { item: body, res: res } });
        }
      }
    } catch (error) {
      console.log(error);
